...
class Interpolation{
    ...
    constructor(subject){
        this.x_bot_animation_obj = new Go_rest_bot(subject);
        this.utils = new Utils();
    }


    init(start_config, end_config, time_interval){
        let array_animation = this.subject_configurations_to_animation(start_config, end_config, time_interval);
        const loop = false;
    ...
    init_v2(array_config, array_time, loop){
        ...
        let array_animation = this.subject_configurations_to_animation_v3(array_config, array_time);
        //const loop = false;
        this.x_bot_animation_obj.init(array_animation, loop);
    }
    ...
    subject_configurations_to_animation_v3(array_config, array_time){
        if(array_time.length != array_time.length){
            console.log("ERROR: not same length");
            return;
        }
        let my_animation_array = {
            hips_position: {
                enable: false,
                func: null,
                tween: null,
        ...
        for (const [key, value] of Object.entries(my_animation_array)) {
            let array_poses_part = this.__array_config_to_array_part(array_config, key);
            this.__poses_to_animation_v3(array_poses_part, array_time, my_animation_array[key].animation);
        }

        return my_animation_array;
    }



    subject_configurations_to_animation(start, end, time_interval){
        let my_animation_array = {
            // HIPS --------------------------------------------
            hips_position: {
                enable: false,
        ...
        for (const [key, value] of Object.entries(my_animation_array)) {
            this.__poses_to_animation(start[key], end[key], time_interval, my_animation_array[key].animation);
        }
